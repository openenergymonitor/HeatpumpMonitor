...
unsigned long lastwdtresettest = 0;
...
void loop() {
  ...
  if (cmd_ready) {
    ...
    if (strcmp(cmd,"on")==0) {
      ...
      digitalWrite(6,LOW);
      Serial.println("heating:0");
    } else {
      if (cmd[0]=='f') {
        // read integer string part
        char setpointstr[9];
        for (int i=1; i<10; i++) {
          setpointstr[i-1] = cmd[i];
        }
        // convert char array to integer
        int setpoint = atoi(setpointstr);
  ...
  if ((now-last)>=9800 || firstrun) {
    ...
    if (MBUS_ENABLE && mbus_reply_received) {
        ...
        if (page==1) {
          Serial.print(F(",SNXstatus:"));
          Serial.print(decode_1byte_bin(16));
          Serial.print(F(",SNXerrors:"));
          Serial.print(decode_2byte_bin(22));
          Serial.print(F(",SNXenergy:"));
          Serial.print(decode_4byte_bin(38));
          Serial.print(F(",SNXvolume:"));
          Serial.print(decode_4byte_bin(44));
          page = 3;
        } else if (page==3) {
...
void wdt_reset_test() {
  //Serial.println(millis()-lastwdtresettest);
  wdt_reset();
  //lastwdtresettest = millis();
}
